...
// Detect if phases are all "single-day" with identical times → days mode
...
export default function PrescriptionForm({ prescription, onClose }) {
  ...
  function pickExistingMed(mi, medId) {
    ...
    if (!found) return
    setMeds((prev) => prev.map((m, i) => i !== mi ? m : { ...m, name: found.name, dosage: found.dosage || '', unit: found.unit, color: found.color }))
  }
        ...
        <form onSubmit={handleSubmit} className="stack stack-lg">
          ...
          <div className="stack stack-md">
            <div className="presc-section-header">
              ...
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => { setMeds((p) => [...p, newMed(startDate)]); setExpandedMed(meds.length) }}>+ Ajouter</button>
            </div>

            {meds.map((med, mi) => (
              <div key={mi} className="presc-med-block">
                <button type="button" className="presc-med-header" onClick={() => setExpandedMed(expandedMed === mi ? -1 : mi)}>
                  ...
                  <div className="presc-med-body stack stack-md">
                    ...
                    {medications.length > 0 && (
                      <div className="form-group">
                        ...
                        <select onChange={(e) => pickExistingMed(mi, e.target.value)} defaultValue="">
                          ...
                          {medications.map((m) => <option key={m.id} value={m.id}>{m.name}{m.dosage ? ` — ${m.dosage}` : ''} ({m.unit})</option>)}
                        </select>
                      </div>
                    )}
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input type="text" placeholder="ex: 500mg..." value={med.dosage} onChange={(e) => setMed(mi, 'dosage', e.target.value)} />
                      </div>
                      <div className="form-group">
                        <label>Unité</label>
